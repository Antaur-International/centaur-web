  ...
  return (
    ...
    >
      ...
      <div className="list_item_bottom">
        <input
          type={"checkbox"}
          checked={props.task.task_status === "pending" ? false : true}
          onClick={(e) => {
            e.stopPropagation();
          }}
        />
        <div className="item_bottom_content">
          ...
          <p className="bottom_content_task">{props.task.task_description}</p>
        </div>
      </div>
    </li>
  );
...
export default function MyWorkMain({
  ...
  const PersonalList =
    personalTaskList.length > 0 ? (
      personalTaskList.map((item, key) => {
        return (
          <MyWorkListItem
            key={key}
            task={item}
            onClick={() =>
              setEditTaskModalOpen((prevState) => ({
                ...prevState,
                isOpen: true,
                task: item,
              }))
  ...
  const CollegeList =
    collegeTaskList.length > 0 ? (
      collegeTaskList.map((item, key) => {
        return (
          <MyWorkListItem
            key={key}
            task={item}
  ...
  const toggleAccordion = () => {
    ...
    setPersonalTaskHeight(
      personalTaskIsOpen ? 0 : `${personalTask.current.scrollHeight}px`
    );
  };

  const toggleCollegeAccordion = () => {
    setCollegeTaskIsOpen(!collegeTaskIsOpen);
    setCollegeTaskHeight(
      collegeTaskIsOpen ? 0 : `${collegeTask.current.scrollHeight}px`
    );
  };

  useEffect(() => {
    setPersonalTaskHeight(
      personalTaskIsOpen ? `${personalTask.current.scrollHeight}px` : 0
    );

    axios
      .get(
        `${API_HOST}/task/personal/${
  ...
  const deleteTask = () => {
    ...
    if (deletedTask.task_cat === "Personal") {
      setPersonalTaskList(
        personalTaskList.filter((item) => item._id !== deletedTask._id)
      );
    } else {
      setCollegeTaskList(
        collegeTaskList.filter((item) => item._id !== deletedTask._id)
      );
    }
  };
    ...
    <section className="wrapper_myWork_main">
      ...
      {isModalOpen && (
        <ModalLayout>
          {" "}
          <AddTaskModal
            setSnackbarOpenState={setSnackbarOpenState}
            openState={openState}
            setMessage={setMessage}
            handleSubmitUpdate={createTask}
      ...
      <ul className="myWork_main_todoCategory">
        <li className="main_todoCategory_item">
          ...
          <ul
            className="todoCategory_item_list"
            ref={personalTask}
        ...
        <li className="main_todoCategory_item">
          <button
            className="todoCategory_item_label"
