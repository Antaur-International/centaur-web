...
export default function EditTaskModal({ setIsOpen, task, handleDeleteUpdate }) {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`${API_HOST}/task/${updateBtn}`, data)
      .then((res) => {
        // console.log(res);
        setIsOpen(false);
        // refresh the page
  ...
  const handleDelete = (e) => {
    ...
    axios
      .delete(`${API_HOST}/task/${deleteBtn}`)
      .then((res) => {
        ...
        <div className="center_modal_content">
          <form className="modal_content_form">
            <select disabled className="cat_select" ref={taskCategory}>
              ...
              <option
                value="Personal"
                selected={task.task.task_cat === "Personal" ? true : false}
              >
                Personal
              </option>
              <option
                value="College"
                selected={task.task.task_cat === "College" ? true : false}
              >
                College
              </option>
            </select>
            ...
            />
            <textarea
              className="content_form_noteInput"
              placeholder="Note..."
            ...
            <div className="content_form_actionBtn">
              ...
              <button
                className="form_actionBtn_addTask btn"
                data-id={task.task._id}
                onClick={handleSubmit}
              >
                Update
              </button>
              <button
                className="form_actionBtn_deleteTask btn"
                data-id={task.task._id}
                onClick={(e) => {
                  handleDelete(e);
                }}
