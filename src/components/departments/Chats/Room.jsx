...
const UserChat = (user) => {
  ...
  return (
    <li className={`section_chats_userChat ${userType}`}>
      <img
        src={user.user.sender.image}
        alt={user.user.sender.name}
        className="chats_userChat_img"
      />
      <div>
        <p className="chats_userChat_name">
          {userType === "self" ? "You" : user.user.sender.name}

          <i className="userChat_name_time">{user.user.time}</i>
        </p>
        <p className="chats_userChat_content">{user.user.message}</p>
      </div>
    </li>
  );
};
        ...
        <div className="room_title_extraIcon">
          ...
          {visible && (
            <div className="title_extraIcon_infoDisplay">
              <p>
                Messages you send in this group are secured with end-to-end
                encryption.
              </p>
            </div>
          )}
      ...
      <section className="wrapper_room_section">
        <div className="room_section_chats" ref={messagesList}>
          <ul className="section_chats_display">
            {chatHistory.map((value, key) => {
              return <UserChat user={value} currentUser={user} />;
            })}
          </ul>
        </div>
        <div className="room_section_inputArea">
          <button
            ...
            className="section_inputArea_emojiBtn"
          >
            <EmojiICon />
          </button>
          ...
          <button
            ...
            className="section_inputArea_sendBtn"
          >
            <SendIcon scale={24} />
          </button>
        </div>
