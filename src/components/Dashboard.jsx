...
const MeetingsListItem = ({ user, meeting }) => {
  ...
  const JoinMeeting = (meet_id) => {
    window.open(
      `https://centaur-meet.onrender.com/${meet_id}?userId=${user._id}`,
      "_blank"
    );
...
const NewsCardItem = (props) => {
  return (
    <li className="wrapper_newsCard_item">
      <div className="newsCard_item_head">
        <p className="item_head_category">{props.category}</p>
      </div>

      <p className="newsCard_item_content">{props.content}</p>
    </li>
  );
};
    ...
    <section className="dashboard">
      ...
      <section className="welcome">
        ...
        <div>
          <h2>
            Good {greeting}, <br />{" "}
            <b style={{ fontSize: "2vw" }}>{userInstance.name}</b>
          </h2>
